...
class ProductServiceTests {
    ...
    @Test
    void getAllProductsButRecalled(){
        ArrayList<Product> allProducts = (ArrayList<Product>) productService.getAllProduct();

        Collection<String> validProducts = new ArrayList<>();
        validProducts.add("apple");
        validProducts.add("cookies");

        Assertions.assertTrue(allProducts.size() == validProducts.size());

        for(int i = 0; i < allProducts.size()-1; i++){
            Assertions.assertTrue(validProducts.contains(allProducts.get(i).getName()));
        }
    }
